...
export default function MultiChoiceFlashGame({ minigameData, onComplete }) {
  // all hooks FIRST
        ...
        <div className="text-center">
          ...
          <div className="flex gap-3">
            {!passed && (
              <button
                onClick={handleRetry}
                className="flex-1 px-6 py-3 bg-orange-600 hover:bg-orange-700 rounded-lg font-bold text-white"
              >
                Retry
              </button>
            )}
            ...
            <button
              ...
              className={`${!passed ? "flex-1" : "w-full"} px-6 py-3 rounded-lg font-bold text-white ${
                passed ? "bg-green-600 hover:bg-green-700" : "bg-gray-600 hover:bg-gray-700"
              }`}
            >
        ...
        >
          ...
          {answered !== null && (
            ...
            >
              ...
              >
                {answered === q.correct_answer ? "✓ Correct!" : "✗ Wrong"}
              </div>

              <div className="text-sm text-gray-300 italic">{q.aira_reaction}</div>

              {q.tip && <div className="text-xs text-gray-400 mt-2">💡 {q.tip}</div>}
            </motion.div>
          )}
        </motion.div>
      </AnimatePresence>
        ...
        {q.choices.map((choice, i) => {
          ...
          return (
            <button
              ...
              className={`
                py-3 rounded-md border text-lg font-bold transition-all
                ${
                  answered === null
                    ...
                    : "opacity-50 bg-black/40 border-white/10"
                }
                ${answered !== null ? "cursor-not-allowed" : ""}
              `}
            >
              {choice}
            </button>
