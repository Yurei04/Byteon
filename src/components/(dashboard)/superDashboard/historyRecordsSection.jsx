...
export default function HistorySection() {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      // Split deleted rows by content_type → tab key
...
function HistoryListRow({ item, mode, ac, isSelected, onClick }) {
  // For deleted rows the timestamp field differs
  const eventAt = mode === "rejected" ? item.reviewed_at : item.deleted_at

  return (
    <button
      onClick={onClick}
      className={`w-full text-left py-4 px-2 m-2 flex items-start gap-3 border transition-all duration-150 group
        border-l-2
        ${isSelected
...
function HistoryDetailPane({ item, mode, type, ac }) {
  // For deleted items, the actual content lives in item.original_data (JSONB)
