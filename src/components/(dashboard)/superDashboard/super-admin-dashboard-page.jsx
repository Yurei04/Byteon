...
import {
  ...
  Activity, Sparkles, Monitor, ChevronDown, Sun, Moon,
} from "lucide-react"
import {
  ...
  return (
    <div>
      <button
        ...
        style={isActive ? {
          background: "linear-gradient(135deg, rgb(var(--brand-500) / 0.22), rgb(var(--brand-600) / 0.14))",
          color: "rgb(var(--text-primary))",
          border: "1px solid rgb(var(--brand-500) / 0.45)",
          boxShadow: "0 0 18px rgb(var(--brand-500) / 0.18), inset 0 1px 0 rgb(var(--brand-500) / 0.2)",
        } : {
      ...
      >
        {isActive && (
          <div
            ...
            style={{
              background: "linear-gradient(to bottom, rgb(var(--accent-500)), rgb(var(--brand-500)))",
              boxShadow: "0 0 8px rgb(var(--brand-500))",
            }}
          />
        )}
        <span className="flex-shrink-0" style={{ color: isActive ? "rgb(var(--brand-500))" : "inherit" }}>
        ...
        <ChevronDown
          ...
          style={{
            color: isActive ? "rgb(var(--brand-500))" : "rgb(var(--text-faint))",
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{ background: isSubActive ? page.accent : "rgb(var(--surface-border))" }}
                />
                {page.label}
                {isSubActive && (
                  <span
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{ color: isActive ? "rgb(var(--brand-500))" : "inherit" }}
      >
        {icon}
      </span>
      ...
      {badge != null && badge > 0 && (
        <span
          className="min-w-[18px] h-[18px] flex items-center justify-center rounded-full text-white text-[10px] font-bold px-1"
          style={{
            background: "linear-gradient(135deg, rgb(var(--accent-500)), rgb(var(--brand-500)))",
            boxShadow: "0 0 8px rgb(var(--brand-500) / 0.6)",
          }}
        >
          {badge > 99 ? "99+" : badge}
        </span>
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        { count: pAnn }, { count: pBlog }, { count: pRes }, { count: pendingOrgCnt },
      ] = await Promise.all([
      ...
      setPendingOrgCount(pendingOrgCnt || 0)
    ...
    >
      <style>{`
        .sa-tab[data-state=active] {
          background: linear-gradient(135deg, rgb(var(--accent-500)), rgb(var(--brand-500))) !important;
          color: #ffffff !important;
          border: none !important;
          box-shadow: 0 2px 12px rgb(var(--brand-500) / 0.5) !important;
        }
        .sa-tab { transition: all 0.2s ease !important; }
        .sa-tab:hover:not([data-state=active]) {
          background: rgb(var(--brand-500) / 0.15) !important;
          color: rgb(var(--text-primary)) !important;
        }
        .sa-scrollbar::-webkit-scrollbar { width: 3px; }
        .sa-scrollbar::-webkit-scrollbar-track { background: transparent; }
        .sa-scrollbar::-webkit-scrollbar-thumb { background: rgb(var(--brand-500) / 0.4); border-radius: 10px; }
        .sa-content-scroll::-webkit-scrollbar { width: 5px; }
        .sa-content-scroll::-webkit-scrollbar-track { background: transparent; }
        .sa-content-scroll::-webkit-scrollbar-thumb { background: rgb(var(--brand-500) / 0.3); border-radius: 10px; }
      `}</style>
      ...
      <aside
        ...
        style={{
          ...
          borderRight: "1px solid rgb(var(--brand-500) / 0.18)",
          backdropFilter: "blur(24px)",
        }}
      >
        <div
          className="absolute top-0 left-0 right-0 h-40 pointer-events-none"
          style={{ background: "linear-gradient(to bottom, rgb(var(--brand-500) / 0.12), transparent)" }}
        />

        {/* Brand */}
        <div
          className="relative flex items-center gap-3 p-5 pb-4"
          style={{ borderBottom: "1px solid rgb(var(--brand-500) / 0.18)" }}
        >
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{
              background: "linear-gradient(135deg, rgb(var(--accent-500)), rgb(var(--brand-500)))",
              boxShadow: "0 4px 14px rgb(var(--brand-500) / 0.5)",
            }}
          >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              ...
              <p className="text-[10px]" style={{ color: "rgb(var(--text-faint))" }}>Full Access</p>
            </div>
          </div>
          <button
        ...
        <div className="px-4 pt-5 pb-2">
          <p
            ...
            style={{ color: "rgb(var(--brand-500) / 0.8)" }}
          >
            Navigation
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header
          ...
          style={{
            ...
            borderBottom: "1px solid rgb(var(--brand-500) / 0.18)",
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 text-xs" style={{ color: "rgb(var(--text-faint))" }}>
              <span style={{ color: "rgb(var(--brand-500))", fontWeight: 600 }}>Super Admin</span>
              <ChevronRight className="w-3 h-3" />
              <span style={{ color: "rgb(var(--text-secondary))" }}>{pageTitles[activeTab]?.title}</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <ShieldCheck className="w-3.5 h-3.5" style={{ color: "rgb(var(--brand-500))" }} />
        ...
        <main
          ...
          style={{
            backgroundImage: `
              radial-gradient(rgb(var(--brand-500) / 0.06) 1px, transparent 1px),
              radial-gradient(circle at 15% 0%, rgb(var(--brand-500) / 0.12) 0%, transparent 40%),
              radial-gradient(circle at 85% 100%, rgb(var(--brand-600) / 0.1) 0%, transparent 45%)
            `,
          ...
          <div className="p-5 sm:p-7 max-w-7xl mx-auto w-full">
            ...
            >
              ...
              <div
                ...
                style={{ background: "linear-gradient(to bottom, transparent, rgb(var(--brand-500) / 0.7), transparent)" }}
              />
            </motion.div>
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="overview" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {[
                        ...
                        <button
                          ...
                          onMouseLeave={e => { e.currentTarget.style.background = "rgb(var(--surface-raised) / 0.4)"; e.currentTarget.style.borderColor = "rgb(var(--surface-border) / 0.3)"; e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "none" }}
                        >
                          <span style={{ color: accent }}>{icon}</span>
                          <span className="text-xs text-center leading-tight" style={{ color: "rgb(var(--text-muted))" }}>
                            {label}
                          </span>
                        </button>
                      ))}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div
                      ...
                      style={{
                        background: pendingCount > 0 ? "rgba(245,158,11,0.08)" : "rgb(var(--surface-raised) / 0.3)",
                        border: `1px solid ${pendingCount > 0 ? "rgba(245,158,11,0.3)" : "rgb(var(--surface-border) / 0.3)"}`,
                      }}
                    >
                      ...
                      <p className="text-3xl font-bold" style={{ color: pendingCount > 0 ? "#fbbf24" : "rgb(var(--text-faint))" }}>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        {[
                          ...
                          { label: "Orgs",    value: stats.totalOrgs,   accent: "rgb(var(--brand-500))" },
                          ...
                          <div key={label} className="text-center">
                            ...
                            <p className="text-[10px] mt-1" style={{ color: "rgb(var(--text-faint))" }}>{label}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                    {[
                      ...
                      <div
                        ...
                        style={{ background: "rgb(var(--surface-raised) / 0.4)", border: `1px solid ${accent}25` }}
                      >
                        <Icon className="w-4 h-4 flex-shrink-0" style={{ color: accent }} />
                        <div>
                          <p className="text-base font-bold leading-none" style={{ color: "rgb(var(--text-primary))" }}>
                            {statsLoading ? "—" : value}
                          </p>
                          <p className="text-[10px] mt-0.5" style={{ color: "rgb(var(--text-faint))" }}>{label}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {activeTab === "website" && (
                <motion.div key="website" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-xs font-semibold" style={{ color: "rgb(var(--text-faint))" }}>Previewing:</span>
              ...
              {activeTab === "org-approvals" && (
                <motion.div key="org-approvals" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                  <div
                    className="rounded-2xl p-5"
                    style={{ background: "rgb(var(--surface-raised) / 0.3)", border: "1px solid rgb(var(--brand-500) / 0.18)" }}
                  >
